...
const NAMESPACE = 'amwell-visit-console';
...
const attachStyles = (hostRef) => {
    ...
    const scopeId = addStyle(( supportsShadow && elm.shadowRoot)
        ? elm.shadowRoot
    ...
    if ( (hostRef.$flags$ & 32 /* hasInitializedComponent */) === 0) {
        ...
        if ( !styles.has(scopeId) && Cstr.style) {
            ...
            if ( cmpMeta.$flags$ & 8 /* needsShadowDomShim */) {
                style = await new Promise(function (resolve) { resolve(require('./shadow-css-15cddae9.js')); }).then(m => m.scopeCss(style, scopeId, false));
            }
...
const patchEsm = () => {
    ...
    if ( !(CSS && CSS.supports && CSS.supports('color', 'var(--c)'))) {
        // @ts-ignore
        return new Promise(function (resolve) { resolve(require(/* webpackChunkName: "stencil-polyfills-css-shim" */ './css-shim-d0251953.js')); }).then(() => {
...
const patchBrowser = () => {
    ...
    }
    {
        ...
        if ( !win.customElements) {
            ...
            // @ts-ignore
            return new Promise(function (resolve) { resolve(require(/* webpackChunkName: "stencil-polyfills-dom" */ './dom-df31b462.js')); }).then(() => opts);
        }
    }
